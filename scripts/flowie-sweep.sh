...
# target has invariant failures — so a scheduler (cron/launchd) can notify
# on nonzero exit.
#
...
claude -p "You are running the scheduled FLOWIE sweep. For EACH target listed below, use the Agent tool with subagent_type 'flowie' (launch them in parallel; a '| lens' suffix on a line names the lens to pass). Then assemble the full FLOWIE REPORT of every target verbatim into one document, preceded by a summary table with columns: target | invariant failures. Do nothing else.
